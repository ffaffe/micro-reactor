...
def runscript(args):
    # run full temp logger #
    if args == 1:
        os.system(main())
    if args == 9:
        os.system(sens_test())
    # if args == 2:
    # function goes here... if button 2 pressed "command=lambda:runscript(1/2/3/4...)" run this function
    # os.system('all_temp_logger.py')
...
def exec_program():
    program1 = filedialog.askopenfilename(initialdir="/", title="Select .exe file", filetypes=[("Executable", '*.exe')])
    os.system(program1)
...
canvas7.create_text(375, 40, fill="orange", font="Times 16 italic bold", text="Cutesy animated koala gif/progress bar"
...
# custom code button
openFile = tk.Button(canvas1, text="Import custom code...", padx=10, pady=5, fg="black", bg="yellow", relief="sunken",
                     command=addCode)
openFile.place(x=860, y=565)

# launch thonny button --> facilitate code edit
openFile = tk.Button(canvas1, text="Launch Thonny IDE...", padx=10, pady=5, fg="black", bg="yellow", relief="sunken")
openFile.place(x=20, y=565)
...
openFile.place(x=200, y=565)

# run sensor test button
st_button = tk.Button(root, text="Run sensor test...", padx=20, pady=5, fg="black", bg="yellow", relief="sunken",
                      activebackground="white", activeforeground="blue", command=lambda: runscript(9))
st_button.place(x=375, y=565)
